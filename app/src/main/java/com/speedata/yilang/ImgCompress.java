...
public class ImgCompress { public static byte[] compressBitmap(Bitmap bmp) {
...
//        logger.d("pic len=" + data.length);
    ...
    public static byte[] getPicByte(Bitmap icon) {
        int width = 200;
        int height = 300;
        byte[] photo_data = ImgCompress.compressBitmap(ImgCompress.reduce(icon, width, height, true));

        Bitmap reduce = null;
        while (photo_data.length > 2046) {
            width = width - 20;
            height = height - 20;
            reduce = ImgCompress.reduce(icon, width, height, true);
            photo_data = ImgCompress.compressBitmap(reduce);
